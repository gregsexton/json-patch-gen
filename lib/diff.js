...
var arrayDiff = function( arr1, arr2, path ) {
    ...
    var constructPath = function ( table ) {
        ...
        while (entry.op !== undefined) {
            if (entry.op === 'i') {
                acc.push({op: 'add', path: i, value: arr2[j-1]});
                j--;
            } else if (entry.op === 'd') {
                acc.push({op: 'remove', path: i-1});
                i--;
            } else if (entry.op === 'r') {
                acc.push({op: 'replace', path: i-1, value: arr2[j-1]});
                i--; j--;
            } else if (entry.op === 'm') {
                i--; j--;
            } else {
    ...
    return constructPath(diff(init(new Array(len), len)));
};
